...
def parse_exhauster_data(mapped_data: ExMapper, index: int):
    ...
    if index in (0, 1):
        oil_pressure_status = "alarm" if indexed_exhauster.oil_data.oil_pressure < 0.5 else "default"
        rotor_current_status = (
            "warning" if indexed_exhauster.electricity_data.rotor_current >= 250 else "default"
        )

    else:
        oil_pressure_status = "alarm" if indexed_exhauster.oil_data.oil_pressure < 0.2 else "default"
        rotor_current_status = (
            "warning" if indexed_exhauster.electricity_data.rotor_current >= 200 else "default"
        )
    ...
    stator_current_status = (
        ...
        if (indexed_exhauster.electricity_data.stator_current >= 280)
            ...
            "warning" if (280 < indexed_exhauster.electricity_data.stator_current >= 230) else "default"
        )
    )

    oil_temp_before_status = (
        "warning" if indexed_exhauster.chiller_data.oil_temp.temperature_before >= 30 else "default"
    )

    oil_temp_after_status = (
        "warning" if indexed_exhauster.chiller_data.oil_temp.temperature_after >= 30 else "default"
    )

    water_temp_before_status = (
        "warning" if indexed_exhauster.chiller_data.water_temp.temperature_before >= 30 else "default"
    )

    water_temp_after_status = (
        "warning" if indexed_exhauster.chiller_data.water_temp.temperature_after >= 30 else "default"
    )
    ...
    return ExhausterInfoResponse(
        ...
        bearings=bearings,
        oil_level=indexed_exhauster.oil_data.oil_level,
        oil_level_status=oil_level_status,
        oil_pressure=indexed_exhauster.oil_data.oil_pressure,
        ...
        oil_temp_before=indexed_exhauster.chiller_data.oil_temp.temperature_before,
        oil_temp_before_status=oil_temp_before_status,
        oil_temp_after=indexed_exhauster.chiller_data.oil_temp.temperature_after,
        oil_temp_after_status=oil_temp_after_status,
        water_temp_before=indexed_exhauster.chiller_data.water_temp.temperature_before,
        water_temp_before_status=water_temp_before_status,
        water_temp_after=indexed_exhauster.chiller_data.water_temp.temperature_after,
        ...
        gas_temp_before=indexed_exhauster.gas_manifold_data.temperature_before,
        gas_underpressure_before=indexed_exhauster.gas_manifold_data.underpressure_before,
    )